...
- ویژگی `Secure` کوکی نشست از پروتکل درخواست (`X-Forwarded-Proto`) تعیین می‌شود؛
  Nginx این header را بازنویسی می‌کند، بنابراین HTTP موقت IP کار می‌کند و HTTPS
  دامنه بدون تغییر دستی، کوکی Secure دریافت می‌کند.
